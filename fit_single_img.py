...
def fit(args):
    ...
    for i in tqdm(range(args.first_nrf_iters)):
        ...
        if i%100 == 0:
            ...
            #rendered_img_specular = reni.to_sRGB(rendered_img_specular, args.tar_size, args.tar_size)
        ...
        writer.add_scalars('losses', {'lm':lm_loss_val.detach(),
                                      'id_reg':id_reg_loss.detach(),
                                      'exp_reg':exp_reg_loss.detach(),
                                      'tex_reg':tex_reg_loss.detach(),
                                      'reni_reg':reni_reg_loss.detach(),
                                      'tex_loss':tex_loss_val.detach(),
                                      'photo_loss':photo_loss_val.detach()}, global_step=i)
    ...
    with torch.no_grad():
        ...
        #If original image cannot fit the padded image, we need to crop the padded image (this should only crop from the padding, not the face)
        rebbox = np.concatenate((np.clip(bbox_offset[[0,2]], a_min=0, a_max=np.max(img_arr.shape[1])), np.clip(bbox_offset[[1,3]], a_min=0, a_max=np.max(img_arr.shape[0]))))
        rebbox = rebbox[[0,2,1,3]]
        clipbox = abs(bbox_offset - rebbox)
        clipbox = np.asarray([clipbox[0], clipbox[1], sidelen - clipbox[2], sidelen - clipbox[3]])
        clipbox[:2] = np.where(clipbox[:2] > 0, clipbox[:2] + 1, clipbox[:2])
        resized_out_img = resized_out_img[clipbox[1]:clipbox[3], clipbox[0]:clipbox[2]]
        resized_mask = resized_mask[clipbox[1]:clipbox[3], clipbox[0]:clipbox[2]]

        composed_img = img_arr.copy().astype(np.uint8)
        composed_face = composed_img[rebbox[1]:rebbox[3], rebbox[0]:rebbox[2], :] * \
            (1 - resized_mask) + resized_out_img * resized_mask
        composed_img[rebbox[1]:rebbox[3], rebbox[0]:rebbox[2], :] = composed_face
