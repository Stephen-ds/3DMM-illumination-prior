...
class BaseReconModel(nn.Module):
    ...
    def init_coeff_tensors(self, id_coeff=None, tex_coeff=None, pose_coeff=None):
        if id_coeff is None:
        ...
        if pose_coeff is None:
            self.pose_tensor = torch.zeros(
                (1, self.pose_dims), dtype=torch.float32,
                requires_grad=True, device=self.device)
        else:
            assert pose_coeff.shape == (1, self.id_dims)
            self.pose_tensor = torch.tensor(
                pose_coeff, dtype=torch.float32,
                requires_grad=True, device=self.device)
