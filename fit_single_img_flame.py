...
def fit(args, benchmark=False, lms=None, outpath=None):
    ...
    Z = nn.Parameter(torch.zeros(1,36, 3).float().to(args.device))
    nonrigid_optimizer = torch.optim.Adam([shape, pose, exp, rot_tensor, trans_tensor, tex, Z], lr=args.nrf_lr)
    ...
    for k in range(200, args.first_nrf_iters):
        losses = {}
        vertices, landmarks2d, landmarks3d = flame(shape_params=shape, expression_params=exp, pose_params=pose)
        #vertices = vertices[:,face_inds,:]
        vertices = vertices * 10
        ...
        embed_face = embed_face[0]
        if embed_face != None:
            embeddings = resnet(embed_face.unsqueeze(0).permute(0,3,1,2))
        else: 
            embeddings = 10000000
        ## loss
        ...
        losses['reflectance'] = losses['reflectance'] + lossfuncs.reflectance_loss(albedos_perm, uv_albedo_mask.flatten(1,2)) * args.rot_reg_w
    ...
    with torch.no_grad():
        ...
        envmap_im = reni_output.view(-1, reni.H, reni.W, 3)
        envmap_im = sRGB_old(envmap_im).squeeze(0).cpu().detach().numpy()*255
        pred_albedo_out = albedos.squeeze(0).permute(1,2,0).cpu().detach().numpy()*255
        pred_albedo_out = cv2.cvtColor(pred_albedo_out, cv2.COLOR_RGB2BGR)
        ...
        out_reni_path = os.path.join(
            args.res_folder, basename + '_envmap.jpg')
        out_pred_albedo_path = os.path.join(
            args.res_folder, basename + '_pred_albedo.jpg')
        
        cv2.imwrite(out_composed_img_path, composed_img[:, :, ::-1])
        cv2.imwrite(out_reni_path, envmap_im)
        cv2.imwrite(out_pred_albedo_path, pred_albedo_out)
